...
PangolinDSOViewer::PangolinDSOViewer(int w, int h, bool startRunThread)
{
	...
	fileKey.open ("/home/harasim/catkin_ws/pangolinviewer.log");
...
void PangolinDSOViewer::printResult(std::string file)
{
	std::string fileKeyName = file + "key";
	std::string fileAllName = file + "all";

	std::ofstream fileKey;
	fileKey.open (fileKeyName.c_str());
	fileKey << std::setprecision(15);

	for(unsigned int i=0;i<keyframes.size();i++)
		{
			fileKey << i <<
			" " << keyframes[i]->camToWorld.translation()[0] << 
			" " << keyframes[i]->camToWorld.translation()[1] <<
			" " << keyframes[i]->camToWorld.translation()[2] << "\n";
		}

	fileKey.close();

	std::ofstream fileAll;
	fileAll.open (fileAllName.c_str());
	fileAll << std::setprecision(15);
...
void PangolinDSOViewer::publishCamPose(FrameShell* frame,
		CalibHessian* HCalib, Eigen::Matrix<Sophus::SE3Group<double>::Scalar, 3, 4>* transformation)
{
    if(!setting_render_display3D) return;
    if(disableAllDisplay) return;
